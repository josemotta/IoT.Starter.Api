...
The Lirc version 0.9.4c is configured ONLY by the file `/boot/config.txt`.
...
#### Net status
	...
	# gateway
	pi@lumi:~ $ ip route | grep default | awk '{print $3}'
	192.168.20.1

	pi@lumi:~ $ ip route
	default via 192.168.20.1 dev eth0 src 192.168.20.102 metric 202
	169.254.0.0/16 dev docker0 proto kernel scope link src 169.254.207.15 metric 204
	169.254.0.0/16 dev vethe744df0 proto kernel scope link src 169.254.210.201 metric 206
	169.254.0.0/16 dev vethcdd940d proto kernel scope link src 169.254.62.15 metric 208
	169.254.0.0/16 dev vethfa1a1c8 proto kernel scope link src 169.254.56.86 metric 210
	172.17.0.0/16 dev docker0 proto kernel scope link src 172.17.0.1
	172.18.0.0/16 dev docker_gwbridge proto kernel scope link src 172.18.0.1 linkdown
	192.168.20.0/24 dev eth0 proto kernel scope link src 192.168.20.102 metric 202

	# ip address
	pi@lumi:~ $ ip -4 addr show dev eth0
	2: eth0: <BROADCAST,MULTICAST,UP,LOWER_UP> mtu 1500 qdisc pfifo_fast state UP group default qlen 1000
		inet 192.168.20.102/24 brd 192.168.20.255 scope global eth0
		   valid_lft forever preferred_lft forever

	pi@lumi:~ $ ip -4 addr show dev eth0 | grep inet
		inet 192.168.20.102/24 brd 192.168.20.255 scope global eth0

	# dns
	pi@lumi:~ $ cat /etc/resolv.conf
	# Generated by resolvconf
	domain rjo.virtua.com.br
